...
export default function CoursesContent() {
   ...
   useEffect(() => {
      ...
      if (uniParam) setSelectedUniversity(uniParam);
      else setSelectedUniversity("all");
   }, [searchParams]);
      ...
      return universitiesData.flatMap((uni) =>
         uni.courses_and_qualifications.map((course) => {
            ...
            if (
               ...
               if (match) numericFee = parseInt(match[1].replace(/,/g, ""), 10);
            }
                        ...
                        >
                           {autocompleteSuggestions.map((title, idx) => (
                              <button
                                 ...
                                 className={`w-full text-left px-4 py-3 text-xs font-medium transition-colors truncate ${selectedIndex === idx ? "bg-blue-600 text-white" : "hover:bg-blue-50 dark:hover:bg-slate-800/80 text-slate-700 dark:text-slate-300"}`}
                              >
                                 {title}
                              </button>
                           ))}
